...
n_input = 3 # rnn input size
n_hidden = 64 # number of rnn hidden units
n_output = 6 # rnn input size
...
if __name__ == "__main__":
    ...
    for epoch in range(Nepochs):
        ...
        if (epoch > 500) and ((np.absolute(loss.data.numpy()) <= 0.0005) or (epoch == 1000)):
            break
    fig = plt.figure()
    ...
    #test
    idx_t = 19
    Xtest_t = Xall[idx_t*(Nrnn+1):(idx_t+1)*(Nrnn+1),:]
    Ytest_t = Yall[idx_t*(Nrnn+1):(idx_t+1)*(Nrnn+1),:]
    Xtest_t = Np2Var(Xtest_t)
    Ytest_t = Np2Var(Ytest_t)
    Xtest_t = Xtest_t.view(Nrnn+1,1,-1)
    Ytest_t = Ytest_t.view(Nrnn+1,1,-1)
    X = Xall[Nrnn+1:,:]
    Y = Yall[Nrnn+1:,:]
    
    net_t = RNN(n_input,n_hidden,n_output,n_layers)
    net_t.load_state_dict(torch.load('data/trained_nets/RNNLorenz.pt'))
    net_t.eval()
    Ypred_t = net_t(Xtest_t).data.numpy()
    Ytest_t = Ytest_t.data.numpy()
    Ypred_t = np.ravel(Ypred_t)
    Ytest_t = np.ravel(Ytest_t)
    dY = []
    for i in range(Ytest_t.shape[0]):
        dY.append((np.linalg.norm(Ypred_t[i]-Ytest_t[i]))**2)
    
    fig = plt.figure()
    plt.plot(np.linspace(0,50,Ytest_t.shape[0]),dY)